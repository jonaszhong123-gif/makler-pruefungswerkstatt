...
function viewFromHash() {
  ...
  return navigation.some((item) => item.id === candidate) ? candidate : 'today'
}
...
function renderToday() {
  ...
  return `<div class="view-shell" data-view="today"><section class="today-lead"><div class="lead-copy"><p class="eyebrow">${escapeHtml(dateLabel)}</p><h1>Heute nicht mehr lernen.<br><em>Besser entscheiden.</em></h1><p>Die Werkstatt trainiert den Weg von der unvollständigen Akte zur begründeten Entscheidung — mit sichtbaren Grenzen statt erfundener Sicherheit.</p><button class="primary-action" type="button" data-action="navigate" data-view="${nextView}">${nextReview ? 'Wiederholung öffnen' : nextLesson ? 'Nächsten Lernschritt öffnen' : 'Fallakte weiterführen'}<span aria-hidden="true">→</span></button></div><aside class="today-brief" aria-label="Heutiger Arbeitsauftrag"><div class="brief-number">01</div><p class="micro-label">HEUTIGER ARBEITSAUFTRAG</p>${brief}<div class="brief-foot">Kein Gesamt-Passwert · kein Prognosewert</div></aside></section><section class="module-ledger" aria-labelledby="module-ledger-title"><div class="section-heading"><p class="micro-label">VIER GETRENNTE GEGENSTÄNDE</p><h2 id="module-ledger-title">Dein Stand, ohne erfundene Gesamtquote</h2></div><div class="ledger-lines">${moduleRows}</div></section><section class="today-lower"><div class="queue-preview"><p class="micro-label">WIEDERHOLUNGSQUEUE</p>${queue}</div><blockquote><p>„Erst verstehen, was den Betrieb handlungsunfähig macht. Dann prüfen, welche Antwort wirklich trägt.“</p><footer>Arbeitsprinzip · ${statusLabel('derived')}</footer></blockquote></section></div>`
}
...
function renderLearningPath() {
  const chapters = modules.map((module, moduleIndex) => {
    ...
    const lessonRows = entries.map((lesson) => {
      ...
      return `<button type="button" class="lesson-line ${done ? 'is-done' : ''}" data-action="toggle-lesson" data-id="${escapeHtml(lesson.id)}" aria-pressed="${done}"><span class="paper-check" aria-hidden="true">${done ? '×' : ''}</span><span class="lesson-copy"><strong>${bilingual(lesson.title)}</strong><small>${bilingual(lesson.purpose)}</small></span>${statusLabel('derived')}</button>`
    }).join('')
...
function renderWritten() {
  ...
  return `<div class="view-shell" data-view="written">${pageHeader('SCHRIFTLICH · ORIGINALE ÜBUNGEN', 'Nicht erraten. Sichtbar herleiten.', 'Vier eigenständige Übungsaufgaben, abgeleitet aus den bestätigten Lernergebnissen. Keine offizielle Prüfungsfrage und keine Erfolgsprognose.', '05')}<div class="practice-layout"><nav class="practice-tabs" aria-label="Schriftliche Aufgaben">${tabs}</nav><article class="practice-sheet"><div class="practice-prompt">${moduleStamp(active.module)}<p class="micro-label">AUFGABE · NICHT OFFIZIELL</p><h2>${escapeHtml(active.title)}</h2><p>${escapeHtml(active.prompt)}</p>${state.chinese ? `<p class="zh-help" lang="zh-Hans">${escapeHtml(active.promptZh)}</p>` : ''}</div><label class="answer-field"><span>Arbeitsantwort <small>lokal gespeichert</small></span><textarea rows="12" data-input="written-answer" data-id="${escapeHtml(active.id)}" placeholder="Trenne Fakten, Ableitungen, unknown und den nächsten Prüfschritt …">${escapeHtml(answer)}</textarea></label><div class="answer-actions"><span id="answer-count">${answer.trim().length === 0 ? 'Noch keine Antwort' : `${answer.trim().length} Zeichen lokal`}</span><button type="button" class="primary-action compact" data-action="written-reveal" ${answer.trim().length < 20 ? 'disabled' : ''}>Referenzpunkte prüfen</button></div>${reference}</article></div></div>`
}
...
function render() {
  const nav = navigation.map((item) => `<button type="button" data-action="navigate" data-view="${item.id}" class="${state.view === item.id ? 'is-active' : ''}" ${state.view === item.id ? 'aria-current="page"' : ''}><span>${item.number}</span><strong>${item.label}</strong></button>`).join('')
  document.querySelector('#root').innerHTML = `<div class="app-frame"><a class="skip-link" href="#main-content">Zum Inhalt springen</a><aside class="app-rail"><header class="brand-block"><span class="brand-kicker">VERSICHERUNGSMAKLER</span><strong>Prüfungs<br>werkstatt</strong><p>Modul 1 / 2 · v0</p></header><nav class="main-nav" aria-label="Hauptnavigation">${nav}</nav><footer class="rail-footer"><button type="button" class="language-toggle" data-action="toggle-chinese" aria-pressed="${state.chinese}"><span>中文 Hilfe</span><i aria-hidden="true">${state.chinese ? 'AN' : 'AUS'}</i></button><p>Deutsch bleibt Normebene.</p><div class="scope-tag">M3 · EXCLUDED</div></footer></aside><main id="main-content" tabindex="-1"><div id="storage-status" aria-live="assertive">${storageErrorMarkup()}</div>${viewRenderers[state.view]()}<footer class="app-footer"><span>Makler Prüfungswerkstatt · lokaler v0</span><span>Keine offizielle Prüfungsplattform · keine Rechts- oder Vertragsberatung</span></footer></main></div>`
}

function navigate(view, moduleCode) {
...
document.addEventListener('click', (event) => {
  ...
  if (action === 'navigate') return navigate(trigger.dataset.view, trigger.dataset.module)
